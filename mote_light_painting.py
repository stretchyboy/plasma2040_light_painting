...
class MoteLightPainting(object):
    ...
    def connectToMote(self):
        ...
        try:
            ...
            self.showMessage("Creating Mote")
            self.mote = Mote()
            self.simulate = False
            self.showMessage("Connected")
            if not self.simulate :
                self.showMessage("Clearing")
                self.mote.clear()
                self.mote.show()
                self.showMessage("Ready")
        except IOError:
            self.simulate = True
            self.showMessage("IOError: Simulating")
    ...
    def show(self):
        ...
        self.showMessage(message)
        print("self.iDuration", self.iDuration)
        duration = float(self.iDuration)#self.scaDuration.get())
    ...
    def doColumn(self):
        if self.gui and time.time() < self.targetTime:
            self.timer = self.delay(1, self.doColumn)
            return
    ...
    def drawCountdown(self):
        ...
        if self.delayRemaining > 0:
            self.delay(1000, self.drawCountdown)
        else:
          self.beep(0.3)
    ...
    def singleShowGui(self):
        ...
        self.delay(delayTime * 1000, self.startPhoto)

        #start the show after waiting for delay & lag
        self.delay((delayTime + lag) * 1000, self.show)
    ...
    def settings(self):
      ...
      return {
      'doQuick':self.doQuick,
      'timeSlices': self.timeSlices,
      'motePixelInSceenPixels':self.motePixelInSceenPixels,
      'graphWidth':self.graphWidth,
      'cameraLag':self.cameraLag,
      'currentColumn':self.currentColumn,
      'simulate':self.simulate,
      'animate':self.animate,
      'completeRepeats':self.completeRepeats,
      'motePixelInCm':self.motePixelInCm,
      'mode':self.mode,
      'color':self.color,
      'colorend':self.colorend,

      'bCameraBusy':self.bCameraBusy,

      'bControlCamera':self.bControlCamera,
      'bPaintFromLeft':self.bPaintFromLeft,
      'bFacingScreen':self.bFacingScreen,
      'bReverseImage':self.bReverseImage,
      'bFlipVertical':self.bFlipVertical,
      'bGradient':self.bGradient,
      'bPaintWhite':self.bPaintWhite,
      'bPaintBlack':self.bPaintBlack,
      'bTween':self.bTween,

      'bLines':self.bLines,
      'bSpeckles':self.bSpeckles,
      'bRandomAcrossRange':self.bRandomAcrossRange,

      'iDuration':self.iDuration,
      'iRepeats':self.iRepeats,
      'iDelay':self.iDelay,
      'motePixelInSceenPixels':self.motePixelInSceenPixels
      }
